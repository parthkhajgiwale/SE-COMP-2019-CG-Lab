...
float dx, dy,m[10],x_int[1@], inter x[10];
initgraph(&gd, &gm, "c://Turboc3//BGI");
cout<<"\n Enter the number of vertices of the graph: “3 cin>>n;
cout<<"\n Enter the vertices: \n":
for(i=0;i<n;i++)
{
cout<<"x"<<i<<":"5 cin>>x[i];
...
if(y[i]<ymin)
	ymin=y[i];
ed[i].xl=x[i]; //ed[0].x1l=x[0] ed[0].y1=y[0];
ed[i].yl=y[i];
}
...
j=0;
for(i=0;i<n; itt) //Finding x intersections
{
if(ed[i].flag==1)
{
if(yy==ed[i].y1)
